...
bool PositionControl::initArm()
{
    ...
    // Write values
    for (int index = 0; index < dxl_cnt_arm_; ++index) {
        // if ((index == 0) || (index == dxl_cnt_arm_ - 1))
        // {
        //   dxl_wb_arm_->itemWrite(dxl_id_arm_[index], "Position_P_Gain", 0);
        //   dxl_wb_arm_->itemWrite(dxl_id_arm_[index], "Position_I_Gain", 0);
    ...
    int index)
{
    ...
    if (index <= 4) {
        ...
        if (use_arm_) {
            ...
            motorPos = (-motorPos - GRIPPER_OPEN_MOTOR_POS) / (GRIPPER_CLOSE_MOTOR_POS - GRIPPER_OPEN_MOTOR_POS) * (GRIPPER_CLOSE_MOVEIT - GRIPPER_OPEN_MOVEIT) + GRIPPER_OPEN_MOVEIT;
        } else {
...
void PositionControl::goalJointPositionCallback(const sensor_msgs::JointState::ConstPtr& msg)
{
    if (use_arm_) {
        if (msg->position.size() != 5) {
            ...
            double goal_position[dxl_cnt_arm_] = {
                0.0,
            };
            ...
            int32_t goal_dxl_position[dxl_cnt_arm_] = {
                0,
            };
